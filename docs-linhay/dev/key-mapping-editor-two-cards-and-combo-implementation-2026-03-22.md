...
3. `NJOutputKeyPress`：
   ...
   - 读取后统一转换为新步骤模型
4. 触发行为：
   - 首步骤在 `delay=0` 时使用“长按语义”：触发时按下，解除触发时抬起
   - 上述长按语义同时覆盖单键与组合键（组合键按下顺序触发、抬起时反向释放）
   - 当存在多步骤时，仅首步骤可进入长按语义；后续步骤始终按点按语义执行
   - 其他步骤（含首步骤 delay>0、以及所有后续步骤）按步骤延时触发并发送按下/抬起
...
3. 最终落地：
   ...
   - 多步骤场景下“方式”列固定约束：仅第 1 步可显示“长按”（需 delay=0），其余步骤固定“点按”。
