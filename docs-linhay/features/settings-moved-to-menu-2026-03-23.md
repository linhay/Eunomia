...
2. `EunomiaInspectorPanelView` 恢复为 3 模块（Status / Mappings / Output Editor）。
3. 新增 `public EunomiaSettingsView` 供 App 场景调用。
4. 在 `EunomiaApp` 增加 `Settings { EunomiaSettingsView() }`。
