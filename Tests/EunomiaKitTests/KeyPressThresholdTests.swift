...
final class KeyPressThresholdTests: XCTestCase {
    ...
    func testHoldSemanticsEnabledForSingleZeroDelayComboStep() {
        ...
        XCTAssertTrue(output.usesHoldSemantics(for: sequence, stepIndex: 0))
    }
    ...
    func testHoldSemanticsDisabledWhenDelayIsPositive() {
        ...
        XCTAssertFalse(output.usesHoldSemantics(for: sequence, stepIndex: 0))
    }

    func testHoldSemanticsEnabledForFirstStepInMultipleSteps() {
        let output = NJOutputKeyPress()
        let sequence = [
    ...
    func testHoldSemanticsDisabledForNonFirstSteps() {
        let output = NJOutputKeyPress()
        let sequence = [
            NJKeySequenceStep(keys: [12], delayMilliseconds: 0),
            NJKeySequenceStep(keys: [13], delayMilliseconds: 0)
        ...
        XCTAssertFalse(output.usesHoldSemantics(for: sequence, stepIndex: 1))
    }

    func testEventFlagsForDeleteStepAreEmpty() {
        let output = NJOutputKeyPress()
        let step = NJKeySequenceStep(keys: [UInt16(kVK_Delete)], delayMilliseconds: 0)

        XCTAssertEqual(output.eventFlagsForStep(step, keyDown: true), CGEventFlags())
        XCTAssertEqual(output.eventFlagsForStep(step, keyDown: false), CGEventFlags())
    }

    func testEventFlagsForCommandDeleteContainCommandMask() {
        let output = NJOutputKeyPress()
        let step = NJKeySequenceStep(keys: [UInt16(kVK_Command), UInt16(kVK_Delete)], delayMilliseconds: 0)

        XCTAssertEqual(output.eventFlagsForStep(step, keyDown: true), .maskCommand)
        XCTAssertEqual(output.eventFlagsForStep(step, keyDown: false), .maskCommand)
    }

    func testEventFlagsForFunctionDeleteOnlySetFnOnKeyDown() {
        let output = NJOutputKeyPress()
        let step = NJKeySequenceStep(keys: [UInt16(kVK_Function), UInt16(kVK_Delete)], delayMilliseconds: 0)

        XCTAssertEqual(output.eventFlagsForStep(step, keyDown: true), .maskSecondaryFn)
        XCTAssertEqual(output.eventFlagsForStep(step, keyDown: false), CGEventFlags())
    }
}
