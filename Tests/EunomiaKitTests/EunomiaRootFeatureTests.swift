...
@MainActor
final class EunomiaRootFeatureTests: XCTestCase {
    private func makeStoreWithDraftSnapshot(
        _ snapshot: LockIsolated<EunomiaRuntimeSnapshot>
    ) -> TestStoreOf<EunomiaRootFeature> {
        var initialState = EunomiaRootFeature.State()
    ...
    private func makeRuntime(
        snapshot: @escaping @Sendable () async -> EunomiaRuntimeSnapshot,
        updates: (@Sendable () async -> AsyncStream<EunomiaRuntimeSnapshot>)? = nil,
        ...
        importMapping: @escaping @Sendable () async -> Void = {},
        exportMapping: @escaping @Sendable () async -> Void = {},
    ...
    func testOnAppearLoadsSnapshot() async {
        let snapshot = EunomiaRuntimeSnapshot(
        ...
        let store = TestStore(initialState: EunomiaRootFeature.State()) {
            EunomiaRootFeature(
                runtime: makeRuntime(
                    snapshot: { snapshot }
                )
    ...
    func testCommitRenameTrimsAndCallsRuntime() async {
        ...
        let store = TestStore(initialState: EunomiaRootFeature.State(mappingName: "Default")) {
            EunomiaRootFeature(
                runtime: makeRuntime(
                    snapshot: { snapshot.value },
                    updates: {
    ...
    func testRuntimeUpdateSyncsRenameDraftWhenNoPendingChanges() async {
        var state = EunomiaRootFeature.State(mappingName: "Default")
    ...
    func testDismissKeyMappingEditorClearsEditorState() async {
        ...
        var featureState = EunomiaRootFeature.State()
    ...
    func testSetInspectorSectionExpandedCollapsesRole() async {
        let store = TestStore(initialState: EunomiaRootFeature.State()) {
            EunomiaRootFeature(
                runtime: makeRuntime(
                    snapshot: { EunomiaRuntimeSnapshot() },
                    updates: {
                        AsyncStream { continuation in
                            continuation.finish()
                        }
    ...
    func testSetMappingDetailExpandedUpdatesState() async {
        let store = TestStore(initialState: EunomiaRootFeature.State()) {
            EunomiaRootFeature(
                runtime: makeRuntime(
                    snapshot: { EunomiaRuntimeSnapshot() },
        ...
        await store.send(.view(.setMappingDetailExpanded(false))) {
            $0.mappingDetailExpanded = false
        }
    }

    func testImportMappingCallsRuntime() async {
        let calls = LockIsolated(0)
        let snapshot = LockIsolated(EunomiaRuntimeSnapshot())
        ...
        await store.send(.view(.importMapping))
        await store.receive(.runtimeUpdated(snapshot.value))
        XCTAssertEqual(calls.value, 1)
    }

    func testExportMappingCallsRuntime() async {
        let calls = LockIsolated(0)
        let snapshot = LockIsolated(EunomiaRuntimeSnapshot())
        ...
        await store.send(.view(.exportMapping))
        await store.receive(.runtimeUpdated(snapshot.value))
        XCTAssertEqual(calls.value, 1)
    }
