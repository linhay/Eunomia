...
class NJMapping: NSObject {
    ...
    class func mapping(withContentsOf url: URL, error: NSErrorPointer) -> Any? {
        do {
            ...
            guard let serialization = obj as? [String: Any],
                ...
                error?.pointee = NSError.error(withDomain: "Eunomia", code: 0, description: NSLocalizedString("invalid mapping file", comment: "error when imported file was JSON but not a mapping"))
                return nil
            }
    ...
    func output(forUID uid: String) -> NJOutput? {
        entries[uid]
    }

    func setOutput(_ output: NJOutput?, forUID uid: String) {
        if let output {
            entries[uid] = output
        } else {
