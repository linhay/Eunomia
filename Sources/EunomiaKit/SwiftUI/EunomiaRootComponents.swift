        ...
        roles.map { role in
            ...
            case .status:
                return InspectorPanelSectionDescriptor(
                    ...
                    subtitleKey: "inspector_status_subtitle",
            ...
            case .mappingManager:
                return InspectorPanelSectionDescriptor(
                    ...
                    subtitleKey: "inspector_mappings_subtitle",
                    symbol: "list.bullet"
                )
            case .outputEditor:
                return InspectorPanelSectionDescriptor(
                    ...
                    subtitleKey: "inspector_output_subtitle",
...
struct EunomiaDetailSplitView<MainWorkspace: View, InspectorPanel: View>: View {
...
struct EunomiaMainWorkspaceView<ControllerCard: View, LiveAxisCard: View>: View {
...
enum SidebarSurfacePolicy {
    static let usesUnifiedBackground = true
}

enum InspectorPanelPresentationPolicy {
    static let usesModernCardLayout = true
    static let sectionSpacing: CGFloat = AppleNativeDesignMetrics.spacingM
}

struct EunomiaInspectorPanelView<StatusCard: View, MappingManagerPanel: View, OutputEditorCard: View>: View {
    let inspectorRoles: [RootInspectorRole]
    let expandedRoles: Set<RootInspectorRole>
    ...
    private func isExpanded(for role: RootInspectorRole) -> Bool {
        expansionState.normalizedExpandedRoles.contains(role)
    }
    ...
    @ViewBuilder
    private func modernSectionCard(for section: InspectorPanelSectionDescriptor) -> some View {
        let expanded = isExpanded(for: section.role)
        NativePanelCard(
            title: L10n.text(section.titleKey),
            symbol: section.symbol,
            surfaceStyle: .solid,
            headerTrailing: {
                Button {
                    onSetExpanded(section.role, !expanded)
                } label: {
            ...
            VStack(alignment: .leading, spacing: AppleNativeDesignMetrics.spacingS) {
                Text(L10n.text(section.subtitleKey))
                    .font(.caption)
                    .foregroundStyle(.secondary)
    ...
    @ViewBuilder
    private var modernInspectorContent: some View {
        if sections.isEmpty {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: InspectorPanelPresentationPolicy.sectionSpacing) {
                    Text(L10n.text("inspector_panel_title"))
                        .font(.headline)
                        .foregroundStyle(.primary)

                    ForEach(sections, id: \.role) { section in
                        modernSectionCard(for: section)
                    }
                }
                .padding(AppleNativeDesignMetrics.spacingM)
                .frame(maxWidth: .infinity, alignment: .leading)
            }
            .safeAreaPadding(.bottom, AppleNativeDesignMetrics.spacingS)
        }
    }

    @ViewBuilder
    private var legacyInspectorList: some View {
        if sections.isEmpty {
            ContentUnavailableView(
                L10n.text("selected_input_placeholder"),
                systemImage: "sidebar.left"
            ...
            List {
                ForEach(sections, id: \.role) { section in
                    Section(isExpanded: Binding(
                        get: { isExpanded(for: section.role) },
                        set: { onSetExpanded(section.role, $0) }
                    )) {
                        VStack(alignment: .leading, spacing: AppleNativeDesignMetrics.spacingS) {
                            sectionContent(for: section.role)
                        }
    ...
    @ViewBuilder
    var body: some View {
        let panelContent = Group {
            if InspectorPanelPresentationPolicy.usesModernCardLayout {
                modernInspectorContent
            } else {
                legacyInspectorList
            }
        }

        if SidebarSurfacePolicy.usesUnifiedBackground {
            panelContent
...
struct EunomiaSettingsCardView: View {
    var body: some View {
        NativePanelCard(
...
public struct EunomiaSettingsView: View {
    public init() {}

    public var body: some View {
        ScrollView {
            EunomiaSettingsCardView()
                .frame(maxWidth: .infinity, alignment: .leading)
...
struct EunomiaDeviceRuntimeSidebarView: View {
...
struct EunomiaStatusCardView: View {
...
struct EunomiaLiveAxisCardView: View {
