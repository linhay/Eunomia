...
public struct EunomiaRootView: View {
    private let store: StoreOf<EunomiaRootFeature>
    private let panelLayout = RootPanelLayout.current

    @MainActor
    public init() {
        store = Store(initialState: EunomiaRootFeature.State()) {
            EunomiaRootFeature()
        }
    }
    ...
    private var deviceRuntimeSidebar: some View {
        EunomiaDeviceRuntimeSidebarView(
            ...
            selectedInputID: binding(
                ...
                send: EunomiaRootFeature.View.setSelectedInput
            ),
            simulatingEvents: binding(
                get: { store.simulatingEvents },
                send: EunomiaRootFeature.View.setSimulatingEvents
            ),
        ...
        if panelLayout.inspectorRoles.contains(.mappingManager) {
            EunomiaMappingSidebarView(
                ...
                mappingName: binding(
                    ...
                    send: EunomiaRootFeature.View.setMappingName
                ),
                renameDraftName: binding(
                    get: { store.mappingRenameDraftName },
                    send: EunomiaRootFeature.View.setMappingRenameDraftName
                ),
                detailExpanded: binding(
                    get: { store.mappingDetailExpanded },
                    send: EunomiaRootFeature.View.setMappingDetailExpanded
                ),
                showsTitle: false,
                ...
                onImportMapping: {
                    sendView(.importMapping)
                },
                onExportMapping: {
                    sendView(.exportMapping)
                },
    ...
    private var outputTypeBinding: Binding<OutputType> {
        binding(
            ...
            send: EunomiaRootFeature.View.setOutputType
        )
    }
    ...
    private var keyCodeBinding: Binding<UInt16> {
        binding(
            ...
            send: EunomiaRootFeature.View.setKeyCode
        )
    }
    ...
    private var keyThresholdBinding: Binding<Double> {
        binding(
            ...
            send: EunomiaRootFeature.View.setKeyThreshold
        )
    }
    ...
    private var mappingIndexBinding: Binding<Int> {
        binding(
            ...
            send: EunomiaRootFeature.View.setMappingIndex
        )
    }
    ...
    private var mouseAxisBinding: Binding<Int32> {
        binding(
            ...
            send: EunomiaRootFeature.View.setMouseAxis
        )
    }
    ...
    private var mouseSpeedBinding: Binding<Double> {
        binding(
            ...
            send: EunomiaRootFeature.View.setMouseSpeed
        )
    }
    ...
    private var mouseButtonBinding: Binding<UInt32> {
        binding(
            ...
            send: EunomiaRootFeature.View.setMouseButton
        )
    }
    ...
    private var scrollDirectionBinding: Binding<Int32> {
        binding(
            ...
            send: EunomiaRootFeature.View.setScrollDirection
        )
    }
    ...
    private var scrollSmoothBinding: Binding<Bool> {
        binding(
            ...
            send: EunomiaRootFeature.View.setScrollSmooth
        )
    }
    ...
    private var scrollSpeedBinding: Binding<Double> {
        binding(
            ...
            send: EunomiaRootFeature.View.setScrollSpeed
        )
    }

    private func binding<Value>(
        get: @escaping () -> Value,
        send action: @escaping (Value) -> EunomiaRootFeature.View
    ) -> Binding<Value> {
        binding(
            get: get,
            set: { value in
    ...
    private func sendView(_ action: EunomiaRootFeature.View) {
        send(.view(action))
    }

    private func send(_ action: EunomiaRootFeature.Action) {
        store.send(action)
    }
}
