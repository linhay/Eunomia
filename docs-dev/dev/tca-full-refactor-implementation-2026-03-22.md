# Enjoyable 全量 TCA 改造实现记录（2026-03-22）

## 关联文档
- Feature：`docs-dev/features/tca-full-refactor-2026-03-22.md`

## 实施摘要
1. 新增根级 reducer：`EnjoyableRootFeature`
   - 统一承载根页面状态（映射、设备树、输出草稿、编辑器弹窗等）。
   - 统一承载根页面交互动作（初始化、重命名、映射管理、输出编辑、快捷配置、弹窗保存/关闭）。
2. 新增运行时桥接：`EnjoyableRuntimeClient` + `EnjoyableRuntimeSnapshot`
   - 将原 `EnjoyableStore` 的能力封装为 reducer 可调用接口。
   - 保留原运行时逻辑，避免一次性重写 `NJInputController` 相关实现。
3. 扩展运行时状态快照能力：
   - `EnjoyableStore.snapshot()`
   - `EnjoyableStore.snapshotStream()`
4. 根视图改造成 Store 驱动：
   - `EnjoyableRootView` 从 `@StateObject EnjoyableStore` 切换为 `StoreOf<EnjoyableRootFeature>`。
...
## 测试（红绿过程）
1. 红灯：先新增 `EnjoyableRootFeatureTests`，验证缺失类型导致编译失败。
...
3. 测试可读性提升：
   - `EnjoyableRootFeatureTests` 新增 `makeRuntime` 工厂，移除重复 mock 样板。
...
1. 去掉 `EnjoyableRuntimeClient` 的冗余显式构造器，改用成员初始化器，减少重复字段搬运。
2. `EnjoyableRootFeature` 为 `OutputDraft` 的简单字段更新新增 keyPath 重载：
...
3. 验证与阻塞：
   - 执行 `swift test --filter EnjoyableRootFeatureTests` 与 `swift test --filter EnjoyableStoreTests` 时，均在依赖 `ComposableArchitecture` 编译阶段触发 Swift 编译器断言：`LocalDiscriminator is set multiple times`。
...
1. `EnjoyableRootFeature` runtime 调用去样板：
...
2. `EnjoyableRootView` 发送入口统一：
   - 新增 `send(_ action: EnjoyableRootFeature.Action)`。
   - Sheet cancel/save、sheet binding 关闭分支改用统一发送入口。
3. 验证与阻塞：
   - `swift test --filter EnjoyableRootFeatureTests` 仍在 TCA 依赖编译阶段触发 `LocalDiscriminator is set multiple times`（`Alert.swift`），未能完成本轮测试门禁。

## 继续简化（2026-03-23 01:50）
1. `EnjoyableRootFeature` 分支风格进一步统一：
...
4. 验证与阻塞：
   - `swift test --filter EnjoyableRootFeatureTests` 仍在 TCA 依赖编译阶段崩溃：`LocalDiscriminator is set multiple times`（`Alert.swift`）。
...
3. 验证与阻塞：
   - `swift test --filter EnjoyableRootFeatureTests` 仍在 TCA 依赖编译阶段触发 `LocalDiscriminator is set multiple times`。
...
2. 验证与阻塞：
   - `swift test --filter EnjoyableRootFeatureTests` 仍触发 `LocalDiscriminator is set multiple times`。
...
1. 按 TDD 补充 `EnjoyableRootFeatureTests`：
...
3. 验证与阻塞：
   - `swift test --filter EnjoyableRootFeatureTests` 仍在 TCA 依赖编译阶段断言失败（`LocalDiscriminator is set multiple times`）。
...
- 修复 `KeyComboField` 编译错误：
  - `EnjoyableRuntime.swift` 增加 `import Carbon` 以提供 `kVK_*` 常量。
