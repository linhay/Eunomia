...
关联需求文档：`docs-dev/features/key-mapping-editor-interaction-refresh-2026-03-21.md`

## 实施范围
1. `Sources/EnjoyableKit/SwiftUI/AppleKeyMappingEditorSheet.swift`
2. `Sources/EnjoyableKit/SwiftUI/EnjoyableRuntime.swift`
3. `Sources/EnjoyableKit/Resources/zh-Hans.lproj/Localizable.strings`
4. `Sources/EnjoyableKit/Resources/en.lproj/Localizable.strings`
5. `Tests/EnjoyableKitTests/KeyMappingEditorStateTests.swift`
...
处理：
1. `Package.swift` 中 `EnjoyableKit` product 从显式 `.static` 改为默认库类型（交由 SwiftPM 选择）。
2. 执行 `xcodebuild -project Enjoyable/Enjoyable.xcodeproj -scheme EnjoyableKit -configuration Debug clean build` 重建预览所需中间产物。
3. 验证目标文件存在：
...
验证：
1. `swift test -q` 通过（57 tests, 0 failed）。
2. `xcodebuild -project Enjoyable/Enjoyable.xcodeproj -scheme EnjoyableKit -configuration Debug build` 成功。
...
2. `EnjoyableRootStyling.swift` 新增 `nativeInputFieldBackground()` 修饰器，集中管理输入底板背景形态。
3. 替换重复实现：
   - `EnjoyableRootView.swift` 的 key binding 与 macro step 输入背景改为 `.nativeInputFieldBackground()`。
...
### 实施
1. `EnjoyableMappingSidebarView.swift`
...
2. 新增测试文件：
   - `Tests/EnjoyableKitTests/MappingSidebarContextActionStateTests.swift`
...
### 实施
1. `EnjoyableRootView.swift`
   - 新增 `MappingManagerLayoutPolicy.usesMainstreamSidebarList = true`。
   - 映射设置面板启用侧边栏模式渲染。
2. `EnjoyableMappingSidebarView.swift`
   ...
   - 保留并延续右键菜单操作与当前行高亮。
3. 新增测试文件：
   - `Tests/EnjoyableKitTests/MappingSidebarSelectionStateTests.swift`

### 验证
1. `swift test --filter MappingSidebarSelectionStateTests -q` 通过。
2. `swift test -q` 通过（65 tests, 0 failed）。
3. `xcodebuild -project Enjoyable/Enjoyable.xcodeproj -scheme EnjoyableKit -configuration Debug build` 成功。
...
1. `EnjoyableMappingSidebarView.swift`
   ...
   - 选中项变化与名称变化时同步草稿。
2. 新增测试：
   - `Tests/EnjoyableKitTests/MappingRenamePanelStateTests.swift`
...
### 实施
1. `EnjoyableRootComponents.swift`
   - 新增 `InspectorPanelSectionDescriptor`，统一 inspector section 描述（role/title/symbol）。
   - `EnjoyableInspectorPanelView` 改为 `List + Section` 结构，替代纯 `ScrollView + VStack` 堆叠。
   - 各 section 行统一 row insets / 分隔线隐藏 / 透明行背景，保证卡片式内容在列表中的一致性。
2. `EnjoyableRootView.swift`
   - 传入 `panelLayout.inspectorRoles`，由 inspector 按角色动态组装 section。
3. 新增测试文件：
   - `Tests/EnjoyableKitTests/InspectorPanelSectionDescriptorTests.swift`
...
1. `EnjoyableRootComponents.swift`
   ...
   - `EnjoyableInspectorPanelView` 改为内部 `HSplitView`：
     - 左侧：`List(selection:) + .sidebar` 导航；
     - 右侧：当前模块标题 + 对应详情内容。
   - 通过 `@State selectedRole` 保持切换状态，并在角色列表变化时自动纠正到有效值。
2. 新增测试文件：
   - `Tests/EnjoyableKitTests/InspectorPanelNavigationStateTests.swift`
