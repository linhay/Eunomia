...
关联需求文档：`docs-dev/features/apple-native-uiux-redesign-2026-03-20.md`

## 实施范围
1. `Sources/EnjoyableKit/SwiftUI/EnjoyableRuntime.swift`
2. `Tests/EnjoyableKitTests/AppleNativeDesignTests.swift`
3. `Sources/EnjoyableKit/Resources/en.lproj/Localizable.strings`
4. `Sources/EnjoyableKit/Resources/zh-Hans.lproj/Localizable.strings`
...
## 兼容与风险
1. 业务逻辑层（`EnjoyableStore`）保持不改，降低功能回归风险。
...
2. 绿灯阶段已完成：
   - 新增 `EnjoyableRootView.swift`，作为新默认根视图。
   - 旧根视图实现已下线，保留 `EnjoyableStore` 与共享运行时模型供新 UI 使用。
...
2. 调整：
   - `ContentView` 默认分支改为 `EnjoyableRootView`。
   - 移除旧入口回退路径，统一走新版根视图。
...
2. TDD：
   ...
   - 实现 `InspectorRuntimeControlState` 与新卡片 `EnjoyableDeviceRuntimeCardView` 后转绿。
3. 实现：
   - `EnjoyableMappingSidebarView` 移除模拟事件与辅助功能权限引导，仅保留映射工具。
   - `EnjoyableInspectorPanelView` 扩展为三段：状态卡、设备运行卡、输出编辑卡。
   - 新增 `EnjoyableDeviceRuntimeCardView`：承载设备树（`OutlineGroup`）、模拟事件开关、权限提示与系统设置跳转。
4. 验证：
...
3. 实现：
   ...
   - 新增 `EnjoyableDeviceRuntimeSidebarView`，将设备树、模拟事件开关、辅助功能权限提示放入 Sidebar。
   - `EnjoyableInspectorPanelView` 中间段改为映射管理面板，承载 `EnjoyableMappingSidebarView`（映射导航+重命名+增删+上下移动）。
   - `EnjoyableMappingSidebarView` 增加可配置样式参数，以便在 Sidebar / Inspector 场景复用。
4. 验证：
...
3. 实现细节：
   ...
   - `EnjoyableDeviceRuntimeSidebarView` 重构为单一 `List`，包含：
...
4. TDD 与验证：
   ...
   - 全量验证：
     ...
     - `xcodebuild ... test -only-testing:EnjoyableTests` 在默认部署目标下失败（工程目标版本不一致）。
     - 使用 `MACOSX_DEPLOYMENT_TARGET=26.2` 覆盖后，`xcodebuild ... test -only-testing:EnjoyableTests` 通过。
...
3. 实现：
   ...
   - `EnjoyableStore.openAccessibilitySettings()` 改为调用上述导航器，并在调用后刷新权限状态。
4. 测试：
...
2. 实现：
   - `EnjoyableDeviceRuntimeSidebarView` 中权限提示改为：
