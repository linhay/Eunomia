...
2. 在 `EnjoyableKit` 内新增一个最小可运行的 TCA Feature（State/Action/Reducer/Store）。
...
- Given 工程已拉取依赖
- When 执行 `swift test`
- Then `ComposableArchitecture` 能被 `EnjoyableKit` 与测试 target 正常编译引用
...
2. 已落地最小 TCA 集成闭环：
   - `EnjoyableKit` target 正式依赖 `ComposableArchitecture`
...
4. 详细阻塞与复现记录见：
   - `docs-dev/dev/tca-integration-blocked-by-swift-toolchain-2026-03-22.md`
